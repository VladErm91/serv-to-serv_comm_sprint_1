...
class RabbitMQConnectionManager:
    ...
    async def start_consuming(self, queue_name, callback) -> None:
        """
        Устанавливает соединение с RabbitMQ.
        """
        try:
            self.connection = await connect_robust(settings.rabbitmq_url, loop=None)
                ...
                async for message in queue_iter:
                    async with message.process():
                        try:
                            ...
                            logger.info("Получено сообщение:", payload)
    ...
    async def establish_connection_with_retry(
        ...
        logger.info("Начинаем попытки подключения к RabbitMQ...")
