...
def create_partition(target, connection, **kw) -> None:
    ...
    connection.execute(
        """CREATE TABLE "users_auth_in_smart" PARTITION OF "history_auth" FOR VALUES IN ('smart')"""
    )
    connection.execute(
        """CREATE TABLE "users_auth_in_mobile" PARTITION OF "history_auth" FOR VALUES IN ('mobile');"""
    )
    connection.execute(
        """CREATE TABLE "users_auth_in_web" PARTITION OF "history_auth" FOR VALUES IN ('web')"""
    )
    connection.execute(
...
def upgrade() -> None:
    ...
    op.add_column('users', sa.Column("vk_user_id", sa.VARCHAR(), autoincrement=False, nullable=True))
    op.add_column('users', sa.Column("is_staff", sa.BOOLEAN(), autoincrement=False, nullable=False))
    op.create_unique_constraint(
        constraint_name="users_vk_user_id_key",
        table_name="users",
        columns=["vk_user_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True,
                  server_default=sa.text("uuid_generate_v4()")),  # если расширение uuid-ossp установлено
